...
document.addEventListener("submit", async (e) => {if (e.target && e.target.id === "schoolForm") {e.preventDefault();  const form = e.target;
    const submitBtn = form.querySelector('.submit-btn'); const originalBtnText = submitBtn ? submitBtn.innerText : "Register School";
    const user = auth.currentUser; if (!user) { alert("User not authenticated. Please log in again.");return;}
    try {if (submitBtn) { submitBtn.disabled = true; submitBtn.innerText = "Registering... ⏳";}
      ...
      const schoolData = {schoolName: rawSchoolName,
        isAtl: document.getElementById("isAtl").checked,
        address: {addressLine: document.getElementById("addressLine").value,
          city: document.getElementById("city").value,
          state: document.getElementById("state").value,
          pincode: document.getElementById("pincode").value},
        incharge: {firstName: document.getElementById("inFirst").value,
          lastName: document.getElementById("inLast").value,
          email: document.getElementById("inEmail").value,
          phone: document.getElementById("inPhone").value},
        principal: {firstName: document.getElementById("prFirst").value,
        ...
        correspondent: {firstName: document.getElementById("coFirst").value,
          lastName: document.getElementById("coLast").value,
          email: document.getElementById("coEmail").value,
          phone: document.getElementById("coPhone").value},boards: boards,
        website: document.getElementById("website").value || "",
        paidSubscription: document.getElementById("paidSubscription").checked,
        updatedAt: serverTimestamp(),createdBy: user.uid,
        createdByRole: localStorage.getItem("role") || "unknown"};
      await setDoc(doc(db, "schools", schoolDocId), schoolData, { merge: true });
      alert("School Registered Successfully 🚀"); form.reset(); } catch (error) {
      console.error("Error adding school:", error);
      alert("Something went wrong. Check your console for details.");} finally {
