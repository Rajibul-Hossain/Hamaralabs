...
function renderFields(role) {
  const templates = {
    student: `<input placeholder="Full Name" type="text" id="name" /> <input placeholder="School Name" type="text" id="schoolName"/>`,
    mentor: `<input placeholder="Full Name" type="text" id="name"/> <input placeholder="School Name" type="text" id="schoolName" />`,
    "atl-incharge": `<input placeholder="Full Name" type="text" id="name"/> <input placeholder="School Name" type="text" id="schoolName" />`,
    admin: `<input placeholder="Full Name" type="text" id="name" />`,
    "super-admin": `<input placeholder="Full Name" type="text" id="name" />`,
    "school-admin": `<input placeholder="Full Name" type="text" id="name" /> <input placeholder="School Name" type="text" id="schoolName" />`
  };
...
async function ensureSchoolExists(schoolName) {
  ...
  await setDoc(schoolRef, { schoolName: schoolName, createdAt: serverTimestamp() }, { merge: true });
  return schoolId;}
async function createMentorAssignment(mentorId, schoolId) {
  ...
  if (snap.empty) {await addDoc(collection(db, "mentorSchoolAssignments"), {mentorId: mentorId,schoolId: schoolId,
      createdAt: serverTimestamp()});}}
async function createInchargeAssignment(inchargeId, schoolId) {
  const q = query(collection(db, "inchargeSchoolAssignments"), 
    where("inchargeId", "==", inchargeId), 
    where("schoolId", "==", schoolId));
  const snap = await getDocs(q);
  if (snap.empty) {await addDoc(collection(db, "inchargeSchoolAssignments"), {inchargeId: inchargeId, schoolId: schoolId,
      createdAt: serverTimestamp()});}}
