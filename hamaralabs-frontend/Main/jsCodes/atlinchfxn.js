...
export async function loadInchargeOverview(db, currentUID, contentArea) {
  contentArea.innerHTML = `<div style="display:flex; justify-content:center; align-items:center; height: 60vh; flex-direction:column; gap: 16px;">
 ...
 <style>@keyframes atlSpin { to { transform: rotate(360deg); } }</style></div>`;
...
if (!userDocSnap.exists()) {contentArea.innerHTML = `<div style="padding:40px; text-align:center; color:#ff3b30; font-family:-apple-system, sans-serif; font-weight:600; background:#fff; border-radius:24px; box-shadow:0 10px 30px rgba(0,0,0,0.05); margin: 20px;">⚠️ User profile not found in database.</div>`;return;}
...
window.showMentorPopup = function(event) {
 ...
 if (event && event.currentTarget) {const rect = event.currentTarget.getBoundingClientRect();
...
contentArea.innerHTML = `${iosCss}<div class="atl-dashboard">
