...
export default async function handler(req, res) {cors(req, res, async () => {if (req.method !== 'POST') {return res.status(405).json({ error: 'Method not allowed' });}
    try {const API_KEY = process.env.gemikey; const { promptText } = req.body;
      ...
      res.status(200).json(data);} catch (error) {console.error(error);
